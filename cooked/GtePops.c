...
static u32 getReciprocal (int n)
{
    // UNR table.
    static u8 unrt[] = {
    ...
    // Reciprocal table (generated).
    static u32 reciprocalTable[0x8000];
    ...
    if (tableCreated == 0) {
        int a3;
        u32 *ptr = reciprocalTable;

        for (a3=0x8000; a3<=0xFFFF; a3++) {
    ...
    return reciprocalTable[n & 0x7FFF];
}

static int CountLeadingZeros (u32 rs)
{
    u32 n, m;
...
void gteRTPS ()
{
    ...
    {
        gteFLAG |= ((u32)gteMAC3 > 0xFFFF) << 18;

        if ((gteMAC3 * 2) > gteH) {
            int zeros;
            gteSZ3 = min (gteMAC3, 0xFFFF);

            zeros = CountLeadingZeros (gteSZ3) - 14;
            FDSZ = 0x80000000 + ((u64)gteH << zeros) * (u64)getReciprocal((gteSZ3 << zeros) / 4);
...

    {
        ...
        Acc.QuadPart = (s64)(gteOFX) + C_MAC1 * FDSZ;           // SX
        SX = (Acc.HighPart << 16) | (Acc.LowPart >> 16);
        C_SX = min (max(SX, -1024), 1023);
        gteFLAG |= (SX != C_SX) << 14;
    
        Acc.QuadPart = (s64)(gteOFY) + C_MAC2 * FDSZ;           // SY
        SY = (Acc.HighPart << 16) | (Acc.LowPart >> 16);
...

    {
        ...
        Acc.QuadPart = (s64)(gteDQB) + gteDQA * FDSZ;

        a0 = ( Acc.HighPart & 0x000fffff) | ( ( Acc.LowPart >> 12) << 20 );
        gteMAC0 = Acc.LowPart;
        if ( ( Acc.LowPart >> 12) < 4097 ) {
            gteIR0 = a0;
        }
        else {
